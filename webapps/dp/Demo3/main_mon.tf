...
resource "google_compute_instance" "vm_instance_mon" {
  name         = "mon"
...
resource "null_resource" "provision-mon" {
  depends_on = [google_compute_instance.vm_instance_mon]

  provisioner "remote-exec" {
    connection {
      host        = "${google_compute_instance.vm_instance_mon.network_interface.0.access_config.0.nat_ip}"
      type        = "ssh"
      user        = var.ssh_user
      private_key = file(var.private_key_path)
      agent       = false
    }
    script = var.mon_script_path
  }
}
