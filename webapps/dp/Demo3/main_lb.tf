...
resource "null_resource" "provision-lb" {
  depends_on = [google_compute_instance.vm_instance_lb]

  provisioner "remote-exec" {
    connection {
      host        = "${google_compute_instance.vm_instance_lb.network_interface.0.access_config.0.nat_ip}"
      type        = "ssh"
      user        = var.ssh_user
      private_key = file(var.private_key_path)
      agent       = false
    }
    script = var.lb_script_path
  }
}
